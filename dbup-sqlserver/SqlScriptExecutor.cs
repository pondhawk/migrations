...
namespace DbUp.SqlServer
{
    ...
    public class SqlScriptExecutor : ScriptExecutor
    {
        ...
        protected override void ExecuteCommandsWithinExceptionHandler(int index, SqlScript script, Action executeCommand)
        {
            ...
            {
                Log().LogError("Sql Server Script {0} block number: {1}; Block line {2}; Procedure {3}; Number {4}; Message: {5}", script.Name, index, sqlException.LineNumber, sqlException.Procedure, sqlException.Number, sqlException.Message);
