...
public class UpgradeEngine
{
    ...
    public virtual DatabaseUpgradeResult PerformUpgrade()
    {
        ...
        try
        {
            ...
            {
                configuration.Log.LogDebug("Beginning database upgrade");
                ...
                configuration.Log.LogDebug("Upgrade successful");
                return new DatabaseUpgradeResult(executed, true, null, null);
            }
        }
        catch (Exception ex)
    ...
    List<SqlScript> GetScriptsToExecuteInsideOperation()
    {
        ...
        var executedScriptNames = new HashSet<string>(configuration.Journal.GetExecutedScripts(), configuration.ScriptNameComparer);
        ...
        using (configuration.ConnectionManager.OperationStarting(configuration.Log, marked))
        {
            try
            {
                ...
                foreach (var script in scriptsToExecute)
                {
                    ...
                    configuration.Log.LogDebug("Marking script {0} as executed.", script.Name);
                    marked.Add(script);
                }

                configuration.Log.LogDebug("Script marking successful.");
