...
public abstract class TableJournal : IJournal
{
    ...
    public string[] GetExecutedScripts()
    {
        return ConnectionManager().ExecuteCommandsWithManagedConnection(dbCommandFactory =>
        {
            ...
            {
                Log().LogDebug("Fetching list of already executed scripts.");
            ...
            {
                Log().LogDebug("Journal table does not exist");
                return new string[0];
            }
        });
    }
    ...
    {
        // Now we could run any migration scripts on it using some mechanism to make sure the table is ready for use.
    }
    ...
    public virtual void EnsureTableExistsAndIsLatestVersion(Func<IDbCommand> dbCommandFactory)
    {
        if (!journalExists && !DoesTableExist(dbCommandFactory))
        {
            Log().LogDebug("Creating the {0} table", FqSchemaTableName);
            ...
            Log().LogDebug("The {0} table has been created", FqSchemaTableName);
    ...
    {
        Log().LogDebug("Checking whether journal table exists");
