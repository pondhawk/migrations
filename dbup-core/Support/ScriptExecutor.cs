...
public abstract class ScriptExecutor : IScriptExecutor
{
    ...
    protected virtual string PreprocessScriptContents(SqlScript script, IDictionary<string, string> variables)
    {
        ...
        if (  !string.IsNullOrWhiteSpace(Schema) && !variables.ContainsKey("schema") )
    ...
    public virtual void Execute(SqlScript script, IDictionary<string, string> variables)
    {
        ...
        try
        {
            ...
            Log().LogInformation("Executed Database Server script '{0}'", script.Name);            
            
            
        }
        catch (DbException sqlException)
        {
            Log().LogError("Script {0} block number: {1}; Message: {2}",script.Name, index, sqlException.Message);
            Log().LogError("{0}", sqlException.ToString());
            throw;
        }
        catch (Exception ex)
        {
            Log().LogError("{0} - {1}", script.Name, ex.ToString());
            throw;
        }
    }
        ...
        do
        {
            if (reader.FieldCount == 0)
            {
                if (reader.RecordsAffected >= 0)
                    Log().LogDebug("RecordsAffected: {0}", reader.RecordsAffected);
                return;
            }
            ...
            Log().LogDebug(delimiterLine);
            Log().LogDebug(format, names.ToArray());
            Log().LogDebug(delimiterLine);
            foreach (var line in lines)
            {
                Log().LogDebug(format, line.ToArray());
            }
